...
export default function AdminCreditsPage() {
  ...
  async function runAction(action) {
    ...
    try {
      ...
      if (!res.ok || !data.ok) throw new Error(data.error || "Admin action failed");
      ...
      setStatus(`Ошибка: ${err?.message || "не удалось обновить баланс"}`);
    } finally {
      setBusy(false);
    }
  }
        ...
        <div style={{ border: "1px solid rgba(168,85,247,.35)", background: "linear-gradient(180deg, rgba(24,24,37,.96), rgba(9,9,16,.96))", borderRadius: 24, padding: 28, boxShadow: "0 24px 80px rgba(0,0,0,.45)" }}>
          <div style={{ display: "flex", justifyContent: "space-between", gap: 16, alignItems: "flex-start", flexWrap: "wrap" }}>
            <div>
              ...
              <p style={{ margin: 0, color: "#94a3b8", fontSize: 14 }}>Эта панель меняет баланс в текущем браузере проекта. Доступ закрыт серверной httpOnly cookie-сессией.</p>
            </div>
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              ...
              <button onClick={logout} style={{ color: "#fecaca", background: "rgba(239,68,68,.12)", fontWeight: 900, border: "1px solid rgba(239,68,68,.25)", borderRadius: 12, padding: "10px 14px", cursor: "pointer" }}>Выйти</button>
            </div>
          </div>
          ...
          <div style={{ marginTop: 28, display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 14 }}>
            ...
            <div style={{ background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.08)", borderRadius: 18, padding: 18 }}>
              <div style={{ color: "#34d399", fontSize: 12, fontWeight: 900, textTransform: "uppercase", letterSpacing: 1 }}>Сессия активна</div>
              <div style={{ color: "#94a3b8", fontSize: 13, marginTop: 10 }}>Если cookie-сессия истечёт, админка автоматически отправит тебя на страницу входа.</div>
            </div>
          </div>

          <label style={{ display: "block", marginTop: 24, color: "#cbd5e1", fontSize: 13, fontWeight: 800 }}>Количество</label>
          ...
          {status && <div style={{ marginTop: 18, color: status.startsWith("Ошибка") ? "#f87171" : "#34d399", fontWeight: 800 }}>{status}</div>}
