...
function readBilling() {
  try {
    ...
    if (!raw) return { tokens: 3, date: today() };
    const data = JSON.parse(raw);
    return {
      tokens: Number.isFinite(Number(data.tokens)) ? Number(data.tokens) : 3,
      date: data.date || today(),
    };
  } catch {
    return { tokens: 3, date: today() };
  }
}
...
export default function AdminPage() {
  const [billing, setBilling] = useState({ tokens: 3, date: today() });
