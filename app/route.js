...
// Лимиты payload для reference / multi-character анализа.
// 10 MB на одного персонажа, до 6 персонажей в одном multi-character запросе.
// Base64 тяжелее оригинального файла примерно на 33%, поэтому общий лимит держим с запасом.
const MAX_REFERENCE_IMAGE_BYTES = 10 * 1024 * 1024;
const MAX_REFERENCE_CHARACTERS = 6;
const MAX_MESSAGE_LENGTH = MAX_REFERENCE_IMAGE_BYTES * MAX_REFERENCE_CHARACTERS + 8 * 1024 * 1024;
...
export async function POST(req) {
  try {
    ...
    if (JSON.stringify(messages).length > MAX_MESSAGE_LENGTH) {
      return new Response(
        JSON.stringify({ error: "Входные данные слишком большие. Лимит: до 10 MB на одного персонажа / до 6 персонажей за запрос." }),
